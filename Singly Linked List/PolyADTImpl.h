/* STRUCTS AND FUNCTION DEFINITIONS FOR POLYNOMIAL LIST ADT PROGRAM*/
...
void insertEnd(struct poly *p,int c,int e){
	...
	temp->t.coeff=c;
	i=p;
	while(i->next!=NULL){			//searching for the last node in the linked list
		i=i->next;
	}
	temp->next=i->next;
	i->next=temp;			//connecting temp as the last element after the existing last element
}
...
struct poly* addPoly(struct poly *poly1,struct poly *poly2){ //continue from here
